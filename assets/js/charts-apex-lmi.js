...
(function () {
    ...
    // Heat map chart
    // --------------------------------------------------------------------
    const heatMapEl = document.querySelector('#lmiHeatMapChart'),
            lmiHeatMapChartConfig = {
                chart: {
                    height: 350,
                ...
                series: [
                    {
                        name: 'LAMINATE PRESS',
                        data: buildDataSeries('laminatepress_date', 'laminatepress_value')
                    },
                    {
                        name: 'SAW/DOOR',
                        data: buildDataSeries('sawdoor_date', 'sawdoor_value')
                    },
                    {
                        name: 'EDGE BANDING',
                        data: buildDataSeries('edgebanding_date', 'edgebanding_value')
                    },
                    {
                        name: 'LAMINATE CLEAN/PACK',
                        data: buildDataSeries('laminatecleanpack_date', 'laminatecleanpack_value')
                    },
                ],
                xaxis: {
                    labels: {
    ...
    if (typeof heatMapEl !== undefined && heatMapEl !== null) {
        const lmiHeatMapChart = new ApexCharts(heatMapEl, lmiHeatMapChartConfig);
        lmiHeatMapChart.render();
    }
